 ...
 *       
 * This version starts nw-1 Worker threads. The Emitter sends 
 * to each Worker a pair of values defining the portion of the 
 * iteration space (sub-range) assigned to the Worker. The first
 * partition is reserved for the Emitter, which after the distribution
 * of the pairs, starts computing the local sum on its partition.
 * Each Worker computes the local sum of its sub-range, then it sends 
 * that value to the Emitter which executes the final sum. 
 *
 */
...
struct Emitter:ff_monode_t<double, pair_t> {
    ...
    pair_t* svc(double *s) {
        if (s==nullptr) {
            const long partsize{num_steps/(nw+1)};
            long remaining{num_steps%(nw+1)} , begin{partsize};
