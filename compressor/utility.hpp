...
//
// This is a modified version of the example3.c file distributed with the
// miniz.c, it inflates/deflates block by block.
// --------------------
...
// This code is a mix of POSIX C code and some C++ library call (mainly for the
// strings).
//
...
static inline bool mapFile(const char fname[],
                           size_t size,
                           unsigned char*& ptr) {
    // open input file.
...
// if removeOrigin is true, the source file will be removed if successfully
// compressed
static inline int compressFile(const char fname[],
                               size_t infile_size,
...
// it returns 0 for success and -1 if something went wrong
// if removeOrigin is true, the input compressed file will be removed if
// successfully uncompressed
static inline int decompressFile(const char fname[],
                                 size_t infile_size,
                                 const bool removeOrigin = REMOVE_ORIGIN) {
    FILE* pInfile;
    FILE* pOutfile;
